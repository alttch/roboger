...
test-single:
	cd tests && DBCONN=postgresql://roboger:123@localhost/roboger CLEANUP= LIMITS=1 pytest -x test.py --log-level DEBUG

test-sqlite:
	cd tests && DBCONN=sqlite:////tmp/roboger-test.db CLEANUP= pytest -x test.py --log-level DEBUG
	cd tests && DBCONN=sqlite:////tmp/roboger-test.db CLEANUP= LIMITS=1 pytest -x test.py --log-level DEBUG
	rm -f /tmp/roboger-test.db

test-mysql:
	cd tests && DBCONN=mysql://roboger:123@localhost/roboger CLEANUP= pytest -x test.py --log-level DEBUG
	cd tests && DBCONN=mysql://roboger:123@localhost/roboger CLEANUP= LIMITS=1 pytest -x test.py --log-level DEBUG

test-postgresql:
	cd tests && DBCONN=postgresql://roboger:123@localhost/roboger CLEANUP= pytest -x test.py --log-level DEBUG
	cd tests && DBCONN=postgresql://roboger:123@localhost/roboger CLEANUP= LIMITS=1 pytest -x test.py --log-level DEBUG
